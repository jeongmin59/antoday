...
const WriteTradingRecord: React.FC<WriteTradingRecordPageProps> = ({
  ...
  const [adjustedPrice, setAdjustedPrice] = useState<number | undefined>(
    undefined
  );
  ...
  const gotowritetradingrecord = async () => {
    ...
    console.log(
      tradingData.stockQuantity,
      selectedOption === "매도" ? 1 : 0,
      tradingData.adjustedPrice,
      selectedCompany?.stockCode,
      formatDateToCustom(selectedDate)
    );
    try {
  ...
  const fetchOwnedCompanies = (page = 0) => {
    axios
      .get(`${import.meta.env.VITE_BACK_API_URL}/api/trade/corp`, {
        params: {
          page: page,
        },
        headers: {
      ...
      axios
        ...
        .then((response) => {
          const companyData = response.data.find(
            (company: Company) => company.corpName === selectedCompany?.corpName
          );
          if (companyData) {
            ...
            console.error(
              "Couldn't find matching company data for selected company."
            );
          }
        })
        .catch((error) => {
  ...
  const handleSearchCompany = useCallback(
    (keyword: string) => {
      if (keyword !== "") {
        axios
          .get(`${import.meta.env.VITE_BACK_API_URL}/api/corp/search`, {
            params: {
              keyword: keyword,
              page: currentPage - 1,
            },
      ...
      <div className={styles.card}>
        ...
        {selectedOption === "매도" && (
          <div className={styles.column}>
            ...
            {!isDropdownOpen ? (
              <FontAwesomeIcon icon={faChevronUp} onClick={toggleDropdown} />
            ) : (
          ...
          selectedOption === "매도" && (
            <div className={styles.ownedCompanies}>
              ...
              <div className={styles.paginationContainer}>
                ...
                <span style={{ fontSize: "var(--font-h2)" }}>
                  {currentPage}
                </span>
                <button
                  className={styles.button}
        ...
        {!isDropdownOpen && selectedCompany && selectedOption === "매도" && (
          <div className={styles.width}>
            ...
            <div className={styles.horizontal3}>
              {selectedOption === "매도" ? (
                <div className={styles.weather}>매도 개수</div>
              ) : (
        ...
        <div className={styles.horizontal2}>
          ...
          <div className={styles.priceAdjust}>
            <button
              ...
              onClick={() => adjustPrice(-getIncrementValue())}
            >
              -
            </button>
        ...
        <div className={styles.buttonsContainer}>
          <button onClick={closeWritePage} className={styles.buttoncancel}>
            취소
          </button>
          <button onClick={handleButtonClick} className={styles.buttonadd}>
            추가
          </button>
        </div>
      </div>
