...
def get_price_info(stock_code: str, target_date: date):
    ...
    df = fdr.DataReader(
        stock_code, start_date.strftime("%Y-%m-%d"), target_date.strftime("%Y-%m-%d")
    )

    # 해당 날짜 종가 반환 (해당 날짜 데이터가 없으면 전일종가 반환)
    price = (
        int(df.iloc[-1]["Close"])
        if target_date.strftime("%Y-%m-%d") in df.index
        else int(df.iloc[-2]["Close"])
    )
...
def get_stock_info(corp_list):
    ...
    with SessionLocal() as db:
        stocks = (
            db.query(Stock)
            .filter(Stock.corp_name.in_(corp["corp_name"] for corp in corp_list))
            .all()
        )
        stock_dict = {
            stock.corp_name: {
